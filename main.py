...
# System prompts
BASE_SYSTEM_PROMPT = """
You are Claude, an AI assistant powered by Anthropic's Claude-3-Sonnet model, specialized in software development with access to a variety of tools and the ability to instruct and direct a coding agent and a code execution one. Your capabilities include:
...
def update_system_prompt(current_iteration: Optional[int] = None, max_iterations: Optional[int] = None) -> str:
    ...
    if automode:
        ...
        return BASE_SYSTEM_PROMPT + file_contents_prompt + "\n\n" + AUTOMODE_SYSTEM_PROMPT.format(
            iteration_info=iteration_info) + "\n\n" + chain_of_thought_prompt
    else:
        return BASE_SYSTEM_PROMPT + file_contents_prompt + "\n\n" + chain_of_thought_prompt


def invoke_bedrock_model_full(model_id: str, prompt: str, max_tokens: int = 8000):
...
def invoke_bedrock_model(model_id: str, prompt: str, max_tokens: int = 8000):
    ...
    body_content_str = response['output']['message']['content'][0]['text']
...
async def chat_with_claude(user_input, image_path=None, current_iteration=None, max_iterations=None):
    ...
    if image_path:
        console.print(
            Panel(f"Processing image at path: {image_path}", title_align="left", title="Image Processing", expand=False,
                  style="yellow"))
        ...
        # Note: Bedrock doesn't support image input directly, so we'll need to handle this differently
        # For now, we'll just add a text description of the image
        current_conversation.append({"role": "user",
                                     "content": f"[Image description: An image was uploaded] User input for image: {user_input}"})
    else:
    ...
    try:
        ...
        full_prompt = [
            {
                "role": "user",
                "content": [{ "text": full_prompt_str}],
            }
        ]
        # MAINMODEL call, which maintains context
        response = invoke_bedrock_model_full(MAINMODEL, full_prompt)

        # Update token usage for MAINMODEL (Note: Bedrock doesn't provide token counts, so we'll estimate)
        main_model_tokens['input'] += len(full_prompt_str.split())
        main_model_tokens['output'] += len(response['output']['message']['content'][0]['text'].split())
    ...
    console.print(
        Panel(files_in_context, title="Files in Context", title_align="left", border_style="white", expand=False))
    ...
    if stop_reason == 'tool_use':
        ...
        for tool_request in tool_requests:
            if 'toolUse' in tool_request:
                ...
                console.print(Panel(f"Tool Used: {tool['name']}", style="green"))
                ...
                if tool['name'] in ['create_file', 'edit_and_apply', 'read_file'] and not tool_result["is_error"]:
                    if 'path' in tool['input']:
                ...
                try:
                    ...
                    tool_response = invoke_bedrock_model_full(TOOLCHECKERMODEL, full_prompt)

                    # Update token usage for tool checker
                    tool_checker_tokens['input'] += tool_response['usage']['inputTokens']
                    tool_checker_tokens['output'] += tool_response['usage']['outputTokens']

                    tool_checker_response = ""
                    tool_response_content = response['output']['message']['content']
                    for tool_content_block in tool_response_content:
                        if  "text" in tool_content_block:
                            tool_checker_response += tool_content_block['text']
                    console.print(
                        Panel(Markdown(tool_checker_response), title="Claude's Response to Tool Result",
                              title_align="left",
...
async def generate_edit_instructions(file_content, instructions, project_context):
    ...
    try:
        ...
        response = invoke_bedrock_model(CODEEDITORMODEL, [
            {
                "role": "user",
            ...
            {
                "role": "user",
                "content": [{"text": "Generate SEARCH/REPLACE blocks for the necessary changes."}]
            }
        ])
        # Update token usage for code editor (Note: Bedrock doesn't provide token counts, so we'll estimate)
        code_editor_tokens['input'] += len(system_prompt.split())
        code_editor_tokens['output'] += len(response.split())

        # Parse the response to extract SEARCH/REPLACE blocks
        edit_instructions = parse_search_replace_blocks(response)
...
async def edit_and_apply(path, instructions, project_context, is_automode=False):
    ...
    try:
        ...
        if edit_instructions:
            console.print(Panel("The following SEARCH/REPLACE blocks have been generated:", title="Edit Instructions",
                                style="cyan"))
...
async def send_to_ai_for_executing(code, execution_result):
    ...
    try:
        ...
        response = invoke_bedrock_model(CODEEXECUTIONMODEL, [
            {
                "role": "user",
                "content": [{ "text": system_prompt}]
            },
            {
                "role": "user",
                "content": [{
                             "text": f"Analyze this code execution from the 'code_execution_env' virtual environment:\n\nCode:\n{code}\n\nExecution Result:\n{execution_result}"}]
            }
        ])

        # Update token usage for code execution (Note: Bedrock doesn't provide token counts, so we'll estimate)
        code_execution_tokens['input'] += len(system_prompt.split()) + len(code.split()) + len(execution_result.split())
        code_execution_tokens['output'] += len(response.split())
...
async def main():
    ...
    console.print(Panel("Welcome to the Bedrock Claude-3.5-Sonnet Engineer Chat with Multi-Agent and Image Support!", title="Welcome", style="bold green"))
